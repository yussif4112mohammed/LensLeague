...
export function LoginPage() {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col pt-16 pb-24 sm:pt-24 sm:px-6 lg:px-8 bg-[url('https://images.unsplash.com/photo-1542038784456-1ea8e935640e?q=80&w=2070&auto=format&fit=crop')] bg-cover bg-center bg-no-repeat relative overflow-y-auto">
      <div className="absolute inset-0 bg-background/85 backdrop-blur-sm z-0 fixed" />
      
      <div className="sm:mx-auto sm:w-full sm:max-w-md relative z-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
        <Link to="/" className="inline-flex items-center gap-2 text-muted-foreground hover:text-foreground transition-colors text-sm font-medium mb-8 ml-4 sm:ml-0">
          <ArrowLeft className="w-4 h-4" />
          Back to Home
        </Link>
        
        <div className="bg-card/80 backdrop-blur-xl border border-border py-8 px-4 shadow-2xl sm:rounded-3xl sm:px-10">
          ...
          {showVerifyNotice ? (
            ...
            <>
              <h2 className="mt-2 text-2xl font-bold tracking-tight text-foreground mb-2">Welcome back</h2>
              <p className="text-sm text-muted-foreground mb-8">Log in to continue to your account.</p>

              <Button 
                onClick={handleGoogleLogin} 
                variant="outline" 
                className="w-full h-12 bg-card border-border text-foreground hover:bg-muted rounded-xl font-semibold mb-6 transition-transform duration-200 ease-out active:scale-[0.96]"
              >
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <Button type="submit" className="w-full h-12 bg-primary text-primary-foreground hover:bg-primary/90 font-bold rounded-xl mt-2 shadow-[0_0_15px_rgba(0,0,0,0.05)] transition-transform duration-200 ease-out active:scale-[0.96]" disabled={loading}>
              ...
              <p className="mt-8 text-center text-sm text-muted-foreground">
                ...
                <Link to="/signup" className="font-semibold text-foreground hover:underline transition-all">Sign up</Link>
              </p>
            </>
          )}
        </div>
        ...
        <div className="bg-card/80 backdrop-blur-xl border border-border py-8 px-4 shadow-2xl sm:rounded-3xl sm:px-10">
          ...
          {showVerifyNotice ? (
            <div className="text-center py-8 animate-in fade-in zoom-in-95 duration-500">
              ...
              <Button onClick={() => window.open('https://mail.google.com', '_blank')} className="w-full h-12 bg-primary text-primary-foreground hover:bg-primary/90 rounded-xl font-bold shadow-sm transition-transform duration-200 ease-out active:scale-[0.96]">
                Open Email App
              </Button>
            </div>
          ) : (
            <>
              ...
              {step === 1 && (
                <div className="animate-in fade-in slide-in-from-right-4 duration-500">
                  <h2 className="text-2xl font-bold tracking-tight text-foreground mb-2">Join LensLeague</h2>
                  <p className="text-sm text-muted-foreground mb-8">Choose how you want to use the platform.</p>
                  ...
                  <div className="space-y-4">
                    <button 
                      ...
                      className={cn(
                        ...
                        role === 'photographer' ? "bg-muted border-border shadow-sm" : "bg-card border-border hover:border-ring hover:bg-muted"
                      )}
                    >
                      ...
                      <div>
                        <div className="font-bold text-foreground mb-1">I'm a Photographer</div>
                        <div className="text-sm text-muted-foreground leading-snug">Build your portfolio, compete in battles, and get discovered by top clients.</div>
                      </div>
                    </button>
                    ...
                    <button 
                      ...
                      className={cn(
                        "w-full flex items-start gap-4 p-5 rounded-2xl border text-left transition-all",
                        role === 'client' ? "bg-muted border-border shadow-sm" : "bg-card border-border hover:border-ring hover:bg-muted"
                      )}
                    >
                      <div className="w-10 h-10 rounded-full bg-muted flex items-center justify-center shrink-0">
                      ...
                      <div>
                        <div className="font-bold text-foreground mb-1">I'm Looking to Hire</div>
                        <div className="text-sm text-muted-foreground leading-snug">Find and book top-ranked professional photographers for your next project.</div>
                      </div>
                    </button>
                  </div>
                </div>
              ...
              {step === 2 && (
                <div className="animate-in fade-in slide-in-from-right-4 duration-500">
                  <h2 className="text-2xl font-bold tracking-tight text-foreground mb-6">Create your account</h2>
                  
                  <Button 
                    onClick={async () => {
                      const { error } = await supabase.auth.signInWithOAuth({ provider: 'google', options: { redirectTo: window.location.origin + `/signup?role=${role}` } });
              ...
              {step === 3 && role === 'photographer' && (
                <div className="animate-in fade-in slide-in-from-right-4 duration-500">
                  <h2 className="text-2xl font-bold tracking-tight text-foreground mb-2">Your style</h2>
                  <p className="text-sm text-muted-foreground mb-6">Pick up to 3 categories. This seeds your initial leaderboard placement.</p>
                  ...
                  <Button onClick={handleFinish} className="w-full h-12 bg-primary text-primary-foreground hover:bg-primary/90 font-bold rounded-xl shadow-[0_0_15px_rgba(0,0,0,0.05)] transition-transform duration-200 ease-out active:scale-[0.96]" disabled={loading}>
                    {loading && <Loader2 className="mr-2 h-4 w-4 animate-spin text-primary-foreground" />}
                    Start Competing
                  </Button>
                </div>
